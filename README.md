...
- [`sync-board`](skills/sync-board/SKILL.md) — move Projects v2 board items between Todo / In Progress / Done without hand-looking-up field/option IDs. Invokable as `/sync-board start #N` / `/sync-board done #N #M`, or to bulk-correct the board when it has drifted out of sync with reality.
